#include "../../blackwell_helpers/clc.cuh"
...
__global__ void _matmul_nvfp4_v4(const __grid_constant__ CUtensorMap tmap_A,
    ...
    __shared__ int clc_trigger_phase, store_done_trigger_phase, clc_bar_phase, safe_store_trigger_phase, compute_phase;
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        while (work_is_there){
            for (int i = 0; i < n_iters; i++) {
                ...
                mbarrier_wait(mbar_mm_ptr + stage_id * 8, mm_phase[stage_id]); mm_phase[stage_id] ^= 1;
            ...
            mbarrier_wait(clc_trigger_ptr, clc_trigger_phase); clc_trigger_phase ^= 1;
            iter++;
        }
    } else if (warp_id == 1 && elect_sync()) {
        int iter = 0;
        while (work_is_there){
            mbarrier_wait(store_done_trigger_ptr, store_done_trigger_phase); store_done_trigger_phase ^= 1;
            for (int i = 0; i < n_iters; i++) {
                int stage_id = i % NUM_STAGES;
                mbarrier_wait(mbar_tma_ptr + stage_id * 8, tma_phase[stage_id]); tma_phase[stage_id] ^= 1;
                compute(i, stage_id);
        ...
        while (work_is_there){
            ...
            if (warp_id == 4 && elect_sync()) compute_phase ^= 1;
            ...
            if (warp_id == 4 && elect_sync()) safe_store_trigger_phase ^= 1;
        }
        if (warp_id == 4)
        ...
        while (work_is_there){
            ...
            mbarrier_wait(clc_bar_ptr, clc_bar_phase); clc_bar_phase ^= 1;
...
void matmul_nvfp4_v4(fp16* c, const uint8_t *sfa, const uint8_t* sfb, const uint8_t* a, const uint8_t* b, int M, int N, int K) {
    ...
    _matmul_nvfp4_v4<BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES>
