...
__global__ void cutlass3x_sm120_nvfp4_gemm_v8(
    ...
    if (warp_id == 8 && elect_sync()) {
        ...
        // mbarrier_init(pingpong_mbar,     128);                   
        // mbarrier_init(pingpong_mbar + 8, 128);                   
        fence_mbarrier_init();
        for (int i = 0; i < NUM_STAGES; i++) {
            mbarrier_arrive_count(compute_mbar + i * 8, 32 * 8);
            // mbarrier_arrive_count(compute_mbar_1 + i * 8, 32 * 4);
        }
        // mbarrier_arrive_count(pingpong_mbar + 8, 128);           
    }
    __syncthreads();
    ...
    auto load_regs = [&](int mma_k, int stage_id, int reg_phase) {
        ...
        for(int mma_m = 0; mma_m < WARP_M / MMA_M; mma_m++){
            ...
            int row = (mma_m * MMA_M + (wid_x + consumer_id * 2) * WARP_M + (lane_id % 16));
    ...
    auto epilogue = [&](int off_m, int off_n, int consumer_id){
        ...
        //                 &blockC[consumer_id * (BLOCK_M / 2) * N + (current_row + i * 8) * N + current_col], 
        ...
        for(int mma_n = 0; mma_n < WARP_N / MMA_N; mma_n++){
            #pragma unroll
            for(int mma_m = 0; mma_m < WARP_M / MMA_M; mma_m++){
                int row = consumer_id * (BLOCK_M / 2) + wid_x * WARP_M + mma_m * MMA_M + (lane_id / 4);
                int col = (lane_id % 4) * 2;
                float2 data0 = make_float2(rC[mma_m][mma_n][0], rC[mma_m][mma_n][1]);
                float2 data1 = make_float2(rC[mma_m][mma_n][2], rC[mma_m][mma_n][3]);
                reinterpret_cast<float2*>(&sC[stage][wid_y * (128 * 8) + row * 8 + col])[0] = data0;
                reinterpret_cast<float2*>(&sC[stage][wid_y * (128 * 8) + (row + 8) * 8 + col])[0] = data1;
            }
            fence_proxy_async();
            bar_sync(1 + wg_id, 128);
            if(warp_id % 4 == 0 && lane_id == 0){
                // tma_store_2d(__cvta_generic_to_shared(sC) + stage * 8192, &tmap_C, off_n + mma_n * MMA_N, off_m);
                // tma_store_2d(__cvta_generic_to_shared(sC) + stage * 8192 + (BLOCK_M * 8) * 4, &tmap_C, off_n + (mma_n + 8) * MMA_N, off_m);
                // cp_async_bulk_commit_group();
                // cp_async_bulk_wait_group<1>();
                tma_store_2d(__cvta_generic_to_shared(sC) + stage * 8192 + consumer_id * 2048, &tmap_C, off_n + mma_n * MMA_N, off_m + consumer_id * (BLOCK_M / 2));
                tma_store_2d(__cvta_generic_to_shared(sC) + stage * 8192 + consumer_id * 2048 + (BLOCK_M * 8) * 4, &tmap_C, off_n + (mma_n + 8) * MMA_N, off_m + consumer_id * (BLOCK_M / 2));
                cp_async_bulk_commit_group();
                cp_async_bulk_wait_group<1>();
            }
            bar_sync(3 + wg_id, 128);
            stage ^= 1;
        }
    };
...
static CUtensorMap make_tma_descriptor_C(void *global_addr, uint64_t M,
                                         ...
                                         uint32_t BLOCK_N) {
    CUtensorMap tma_desc;
    uint64_t globalDim[2] = {N, M};
    uint64_t globalStrides[1] = {N * 4};
    uint32_t boxDim[2] = {8, BLOCK_M / 2};
    uint32_t elementStrides[2] = {1, 1};
    cuTensorMapEncodeTiled(
        &tma_desc, CU_TENSOR_MAP_DATA_TYPE_FLOAT32, 2, global_addr, globalDim,
        globalStrides, boxDim, elementStrides, CU_TENSOR_MAP_INTERLEAVE_NONE,
...
torch::Tensor nvfp4_gemm_v8(const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& sfa, const torch::Tensor& sfb){
    int M = a.size(0);
    int N = b.size(0);
    int K = a.size(1) * 2;
    ...
    cutlass3x_sm120_nvfp4_gemm_v8<BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES><<<grid, block>>>(sfa.view(torch::kUInt8).data_ptr<uint8_t>(), sfb.view(torch::kUInt8).data_ptr<uint8_t>(), tma_A, tma_B, tma_C, M, N, K);
    return c;
}