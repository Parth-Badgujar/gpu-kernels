...
__global__ void cutlass3x_sm120_nvfp4_gemm_v6(
    ...
    auto load_smem = [&](int iter_k, int stage_id, int off_n, int off_m){
        tma_load_2d(sA + stage_id * STAGE_SIZE,   &tmap_A, load_mbar + (stage_id << 3), iter_k * BLOCK_K, off_m);
        tma_load_2d(sB + stage_id * STAGE_SIZE,   &tmap_B, load_mbar + (stage_id << 3), iter_k * BLOCK_K, off_n);
        tma_load_flat(sSFA + stage_id * STAGE_SIZE, gSFA + off_m * (K / 16) + iter_k * (BLOCK_M * BLOCK_K / 16), sSFA_mem, load_mbar + (stage_id << 3), CU_TENSOR_MAP_L2_PROMOTION_L2_256B);
        tma_load_flat(sSFB + stage_id * STAGE_SIZE, gSFB + off_n * (K / 16) + iter_k * (BLOCK_N * BLOCK_K / 16), sSFB_mem, load_mbar + (stage_id << 3), CU_TENSOR_MAP_L2_PROMOTION_L2_256B);
    };
    ...
    auto load_regs = [&](int mma_k, int stage_id, int reg_phase){
        ...
        int row_a = (lane_id & 0xf) + (wid_x * WARP_M);
        int col_a = mma_k * (MMA_K / 2) + (lane_id >> 4) << 4;
        int row_b = (lane_id & 0x7) + (wid_y * WARP_N);
        int col_b = mma_k * (MMA_K / 2) + (lane_id >> 3) << 4;
        #pragma unroll
        for(int mma_m = 0; mma_m < WARP_M / MMA_M; mma_m++){
            // int col = mma_k * (MMA_K / 2) + (lane_id / 16) * 16;
            // int row = (mma_m * MMA_M + wid_x * WARP_M + (lane_id % 16));
            ldmatrix_m8n8_x4_b16(rA[reg_phase][mma_m], swizzle<3, 4, 3>(sA_curr + row_a * (BLOCK_K / 2) + col_a));
            row_a += (MMA_M);
        }
        #pragma unroll
        for(int mma_n = 0; mma_n < WARP_N / MMA_N; mma_n++){
            ...
            ldmatrix_m8n8_x2_b16(rB[reg_phase][mma_n], swizzle<3, 4, 3>(sB_curr + row_b * (BLOCK_K / 2) + col_b));
            row_b += (MMA_N);
        }
    };
    ...
    auto store = [&](float* blockC){
        #pragma unroll
        for(int mma_m = 0; mma_m < WARP_M / MMA_M; mma_m++){
            int current_row = (lane_id >> 2) + (mma_m * MMA_M); 
            #pragma unroll
            for(int mma_n = 0; mma_n < WARP_N / MMA_N; mma_n++){
                int current_col = ((lane_id & 3) << 1) + (mma_n * MMA_N); 
                #pragma unroll
                for(int i = 0; i < 2; i++){
                    streaming_store_f32x2(
                        &blockC[((current_row + (i << 3)) * N) + current_col], 
                        make_float2(rC[mma_m][mma_n][(i << 1)], rC[mma_m][mma_n][(i << 1) + 1])
                    );
                }
            }
        }
    ...
    if (warp_id < 4){
        while(bid < NUM_BLOCKS){
            ...
            for(int iter_k = 0; iter_k < K / BLOCK_K; iter_k ++){
                ...
                stage_id = (stage_id + 1) & (NUM_STAGES - 1);
            }
        ...
        while(bid < NUM_BLOCKS){
            ...
            int off_n = coords.y * BLOCK_N;
            for(int iter_k = 0; iter_k < K / BLOCK_K; iter_k ++){
                mbarrier_wait(compute_mbar + (stage_id << 3), (compute_phase >> stage_id) & 1);
                compute_phase ^= (1 << stage_id);
                if (elect_sync()){
...
torch::Tensor nvfp4_gemm_v6(const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& sfa, const torch::Tensor& sfb){
    ...
    cutlass3x_sm120_nvfp4_gemm_v6<BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES><<<grid, block>>>(sfa.view(torch::kUInt8).data_ptr<uint8_t>(), sfb.view(torch::kUInt8).data_ptr<uint8_t>(), tma_A, tma_B, c.data_ptr<float>(), M, N, K);
    return c;
}