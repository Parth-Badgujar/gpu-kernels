...
__device__ __forceinline__ uint32_t swizzle_1_4_3(uint32_t addr){
    constexpr uint32_t bit_msk = (1 << 1) - 1; 
    constexpr uint32_t yyy_msk = bit_msk << (4 + 3); 
    uint32_t shift_val = (addr & yyy_msk) >> 3;
    return addr ^ shift_val;
}


template<int BLOCK_M = 128, int BLOCK_N = 128, int BLOCK_K = 256, int MMA_M = 32, int MMA_N = 32, int NUM_STAGES = 1>
__global__ void _nvfp4_gemm_v4(
...
torch::Tensor nvfp4_gemm_v4(const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& sfa, const torch::Tensor& sfb){
    ...
    _nvfp4_gemm_v4<BLOCK_M, BLOCK_N, BLOCK_K, 32, 32, NUM_STAGES><<<grid, block>>>(sfa.view(torch::kUInt8).data_ptr<uint8_t>(), sfb.view(torch::kUInt8).data_ptr<uint8_t>(), tma_A, tma_B, c.data_ptr<float>(), M, N, K);
    return c;
}