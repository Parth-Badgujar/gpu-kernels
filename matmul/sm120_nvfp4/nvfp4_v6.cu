...
__global__ void _nvfp4_gemm_v6(
    ...
    __shared__ alignas(128) uint32_t ping_pong_ptr[2];
    ...
    uint32_t ping_pong = __cvta_generic_to_shared(ping_pong_ptr);
    if (warp_id == 0 && elect_sync()) {
        for(int i = 0; i < NUM_STAGES; i++){
            mbarrier_init(compute_mbar + i * 8, 32 * 4);
            mbarrier_init(load_mbar + i * 8, 1);
            mbarrier_init(ping_pong, 32 * 4);
        }
        fence_mbarrier_init();
        for(int i = 0; i < NUM_STAGES; i++)
    ...
    if (warp_id < 4){
        ...
        while(bid < NUM_BLOCKS){
            mbarrier_wait(ping_pong, pp_phase); pp_phase ^= 1;
        ...
        mbarrier_arrive(ping_pong);
        while(bid < NUM_BLOCKS){
            mbarrier_wait(ping_pong + 8, pp_phase); pp_phase ^= 1;
            int2 coords = get_grid_coords(bid, N_BLOCKS);
            int off_m = coords.x * BLOCK_M;
            int off_n = coords.y * BLOCK_N;
            clear_acc();
