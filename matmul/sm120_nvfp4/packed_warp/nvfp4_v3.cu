...
__device__ __forceinline__ uint32_t swizzle_3_4_3(uint32_t offset) {
    uint32_t bit_msk = (1 << 3) - 1; 
    uint32_t yyy_msk = bit_msk << (4 + 3); 
...
__global__ void _nvfp4_gemm_v3(
...
)
{
    ...
    auto compute = [&](int iter_k, int stage){
        ...
        for(int mma_k = 1; mma_k < TILES_K; mma_k++){
            ...
            #pragma unroll
            for (int mma_m = 0; mma_m < TILES_M; mma_m += 2) {
                rSFA[reg_stage ^ 1][mma_m / 2] = sfa_ptr[(warp_id / 4) * 64 + ((quad_id) + (lane_id_in_quad & 1) * 8) * 4 + (lane_id_in_quad >> 1) + mma_m];
            }
...
torch::Tensor nvfp4_gemm_v3(const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& sfa, const torch::Tensor& sfb){
    ...
    _nvfp4_gemm_v3<BLOCK_M, BLOCK_N, BLOCK_K, 32, 32, NUM_STAGES><<<grid, block>>>(sfa.view(torch::kUInt8).data_ptr<uint8_t>(), sfb.view(torch::kUInt8).data_ptr<uint8_t>(), tma_A, tma_B, c.data_ptr<float>(), M, N, K);
    return c;
}