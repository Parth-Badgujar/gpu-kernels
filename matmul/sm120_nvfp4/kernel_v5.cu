...
int2 get_grid_coords(int bid, int N_BLOCKS) {
...
__global__ void kernel_v5(
    ...
    auto load_regs = [&](int mma_k, int stage_id, int reg_phase) {
        ...
        for (int mma_m = 0; mma_m < WARP_M / MMA_M; mma_m++) {
            ...
            ldmatrix_m8n8_x4_b16(
                rA[reg_phase][mma_m],
                swizzle<3, 4, 3>(sA_curr + row * (BLOCK_K / 2) + col));
        }
        #pragma unroll
        for (int mma_n = 0; mma_n < WARP_N / MMA_N; mma_n++) {
            ...
            ldmatrix_m8n8_x2_b16(
                rB[reg_phase][mma_n],
                swizzle<3, 4, 3>(sB_curr + row * (BLOCK_K / 2) + col));
        }
    };
    ...
    const int N_BLOCKS = N / BLOCK_N;
    const int M_BLOCKS = M / BLOCK_M;
    ...
    if (WARP_ID < 4) {
        ...
        while (bid < NUM_BLOCKS) {
            ...
            store(gC + off_m * N + off_n); // only changing block offset in gC
            bid += gridDim.x;
        }
    } else if (WARP_ID == 4) {
        while (bid < NUM_BLOCKS) {
            ...
            for (int iter_k = 0; iter_k < K / BLOCK_K; iter_k++) {
                ...
                if (elect_sync()) {
                    ...
                    load_smem(iter_k, stage_id, off_m, off_n);
                }
                stage_id = (stage_id + 1) % NUM_STAGES;
            }
            bid += gridDim.x;
...
torch::Tensor nvfp4_gemm_v5(const torch::Tensor &A, const torch::Tensor &B,
    ...
    dim3 grid(70, 1, 1);
    dim3 block(5 * 32, 1, 1); // 4 compute + 1 load
    ...
    kernel_v5<BLOCK_M, BLOCK_N, BLOCK_K, NUM_STAGES><<<grid, block>>>(
