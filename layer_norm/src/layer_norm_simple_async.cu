                                         ...
                                         size_t stride_b)
{
    ...
    if (threadIdx.x == 0) {
        init(&bar[0], BLOCK_SIZE);
        init(&bar[1], BLOCK_SIZE);
    ...
    float mean_acc = 0.0f, var_acc = 0.0f;

    for (int i = 0; i < LOOPS; ++i) {
        int stage      = i & 1;
        int next_stage = (i + 1) & 1;

        if (i < LOOPS - 1) {
            if (threadIdx.x == 0) {
                token[next_stage] = cuda::device::barrier_arrive_tx(
                    bar[next_stage], 1, aligned_mem);
        ...
        if (global_idx < num_vectors) {
            float4 x = reinterpret_cast<float4*>(&smem[stage * SMEM_STRIDE])[threadIdx.x];
            mean_acc += x.x + x.y + x.z + x.w;
            var_acc  += x.x*x.x + x.y*x.y + x.z*x.z + x.w*x.w;
        }
    }
    ...
    for (int i = 0; i < LOOPS; ++i) {
        ...
        if (i < LOOPS - 1) {
            if (threadIdx.x == 0) {
                ...
                cuda::device::memcpy_async_tx(smem       + next_stage * SMEM_STRIDE,
                                              X_block + (i + 1) * SMEM_STRIDE,
                                              aligned_mem, bar[next_stage]);
                cuda::device::memcpy_async_tx(gamma_smem + next_stage * SMEM_STRIDE,
                                              gamma   + (i + 1) * SMEM_STRIDE,
                                              aligned_mem, bar[next_stage]);
                cuda::device::memcpy_async_tx(beta_smem  + next_stage * SMEM_STRIDE,
                                              beta    + (i + 1) * SMEM_STRIDE,
                                              aligned_mem, bar[next_stage]);
            } else {
                token[next_stage] = bar[next_stage].arrive();
            }
        ...
        float4 x = reinterpret_cast<float4*>(&smem      [stage * SMEM_STRIDE])[threadIdx.x];
        float4 g = reinterpret_cast<float4*>(&gamma_smem[stage * SMEM_STRIDE])[threadIdx.x];
        float4 b = reinterpret_cast<float4*>(&beta_smem [stage * SMEM_STRIDE])[threadIdx.x];
        float4 y;
                             ...
                             size_t B, size_t F, size_t D1, size_t D2)
{
    ...
    switch (BLOCK_SIZE) {
        case 32:
            cudaFuncSetAttribute((const void*)_layer_norm_simple_async<32,  FACTOR>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SMEM_BYTES);
            _layer_norm_simple_async<32,  FACTOR><<<grid, block, SMEM_BYTES>>>(Y, X, gamma, beta, B, F, D1, D2, STRIDE); break;
        case 64:
            cudaFuncSetAttribute((const void*)_layer_norm_simple_async<64,  FACTOR>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SMEM_BYTES);
            _layer_norm_simple_async<64,  FACTOR><<<grid, block, SMEM_BYTES>>>(Y, X, gamma, beta, B, F, D1, D2, STRIDE); break;
        case 128:
            cudaFuncSetAttribute((const void*)_layer_norm_simple_async<128, FACTOR>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SMEM_BYTES);
            _layer_norm_simple_async<128, FACTOR><<<grid, block, SMEM_BYTES>>>(Y, X, gamma, beta, B, F, D1, D2, STRIDE); break;
        case 256:
            cudaFuncSetAttribute((const void*)_layer_norm_simple_async<256, FACTOR>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SMEM_BYTES);
            _layer_norm_simple_async<256, FACTOR><<<grid, block, SMEM_BYTES>>>(Y, X, gamma, beta, B, F, D1, D2, STRIDE); break;
        case 512:
            cudaFuncSetAttribute((const void*)_layer_norm_simple_async<512, FACTOR>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SMEM_BYTES);
            _layer_norm_simple_async<512, FACTOR><<<grid, block, SMEM_BYTES>>>(Y, X, gamma, beta, B, F, D1, D2, STRIDE); break;
        case 1024:
            cudaFuncSetAttribute((const void*)_layer_norm_simple_async<1024, FACTOR>, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)SMEM_BYTES);
            _layer_norm_simple_async<1024, FACTOR><<<grid, block, SMEM_BYTES>>>(Y, X, gamma, beta, B, F, D1, D2, STRIDE); break;
    }
}